...
export default function ChatLayout({
  ...
  const loadConversation = async (conversationId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api${apiEndpoint}/conversations/${conversationId}`
      );
      const data = await response.json();
      if (data.messages) {
      ...
      console.log(
        "Finished loading conversation, setting isConversationLoading to false"
      );
      setIsConversationLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "load-conversation",
      handleLoadConversation as EventListener
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log(
      `Submitting message to ${apiEndpoint}, currentConversationId:`,
    ...
    if (!activeConversationId) {
      console.log(
        `No active conversation for ${apiEndpoint}, creating new one`
      );
      try {
        const response = await fetch(`/api${apiEndpoint}/conversations`, {
          method: "POST",
        ...
        if (data.conversation) {
          ...
          console.error(
            `Failed to create conversation for ${apiEndpoint}`,
            data
          );
    ...
    try {
      console.log(
        `Sending message to ${apiEndpoint} with conversationId:`,
          ...
          {isConversationLoading ? (
            ...
            <div className="h-full flex items-center justify-center relative z-10">
              <div className="relative max-w-md rounded-3xl bg-white/90 dark:bg-gray-800/90 shadow-xl backdrop-blur border border-gray-100/50 dark:border-gray-700/50 p-8 transform transition-all hover:scale-[1.01] duration-300 overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-2xl font-bold mb-3 text-gray-800 dark:text-gray-100 text-center">
                    Welcome to{" "}
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 text-center">
                    Your AI assistant can answer questions based on your
                    uploaded documents.
                  </p>
            ...
            <div className="space-y-6 pb-2 relative z-10">
              {messages.map((msg, index) => {
                ...
                const showTimestamp =
                  isLastMessage || messages[index + 1]?.role !== msg.role;
                ...
                return (
                  <div
                    ...
                    className={`${
                      isUser
                        ? "user-message-enter ml-12"
                        : "bot-message-enter mr-12"
                    }`}
                  >
                    <div
                    ...
                    >
                      ...
                      >
                        <div className="prose prose-blue dark:prose-invert max-w-none [&>p]:mb-2 last:[&>p]:mb-0">
                          <ReactMarkdown
